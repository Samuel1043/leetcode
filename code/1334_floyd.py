...
def FloydWarshall(dp,n):
    ...
    """
    # !!!routing through most be in the outermost for loop
    for k in range(n):
        for i in range(n):
            for j in range(n):
...
def findTheCity(n: int, edges: List[List[int]], distanceThreshold: int) -> int:
    ...
    dp= FloydWarshall(dp,n)
    
    min_cnt=inf
    for idx,vertice in enumerate(dp):
        pass_cnt=0
        for shortest in vertice:
            if shortest<=distanceThreshold:
                pass_cnt+=1
        if idx==38 or idx ==3:
            print(pass_cnt)
        if pass_cnt<=min_cnt:
...
# test case that show outermost should be route through idx (k)
findTheCity(n = 39,
